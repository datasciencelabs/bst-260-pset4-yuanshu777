...
request <- request(url) |>
  req_url_query(
    get = "POP_2020,POP_2021,NAME",  # request 2020/2021 population and state name
    'for' = "state:*",               # fetch data for all states
    key = census_key                  # use Census API key
  )
...
```{r}
response <- request |> req_perform() # send the request and get response
...
```{r}
# parse JSON response and combine rows into a data frame
...
population <- population |> 
  ...
  row_to_names(row_number = 1) |>         # set first row as column names
  select(-state) |>                       # drop raw state code column
  rename(state_name = NAME) |>            # rename NAME column for clarity
  pivot_longer(cols = starts_with("POP_"),
               names_to = "year",
               values_to = "population"   # reshape population columns to long format
               ) |>
  mutate(year = str_remove(year, "POP_"), # clean year labels
         population = as.numeric(population)) |> 
  mutate(state_name = as.character(state_name),   # ensure state_name is character
         state = case_when(                      # add state abbreviations
...
population |>
  ggplot(aes(x = population,
             ...
             fill = year)) +
  geom_col(color = "darkgoldenrod",
           linewidth = 0.3) +   # bar chart of population by state
  facet_wrap(~ year) +          # separate panels for each year
  theme(axis.text.y = element_text(hjust = 1, size = 4)) + # adjust y-axis labels
...
regions <- map_dfr(1:nrow(regions), ~ tibble(
  ...
  mutate(region_name = if_else(region_name ==
    ...
    "NY + Territories", region_name     # simplify long region label
  )) |>
  filter(state_name %in% c(state.name, "District of Columbia", "Puerto Rico"))  
  # keep only U.S. states, DC, and Puerto Rico
...
```{r}
population <- population |>
  left_join(regions, by = "state_name")  # add region info to population data
...
cases_raw <- request(api) |> 
  req_perform() |> 
  resp_body_json(simplifyDataFrame = TRUE)  # fetch and parse raw CDC case data
...
api <- "https://data.cdc.gov/resource/pwn4-m3yp.json"
cases_raw <- request(api) |> 
  req_url_query(`$limit` = "10000000000") |>          # request all rows from API
  req_perform() |> 
  resp_body_json(simplifyDataFrame = TRUE) |>         # parse JSON into data frame
  mutate(
    date  = as.Date(end_date),                        # convert date
    cases = as.numeric(new_cases)                     # convert case counts to numeric
  ) |>
  filter(state %in% c(state.abb, "DC", "PR")) |>      # keep valid states/DC/PR
  select(state, date, cases)                          # keep relevant columns
...
cases <- cases_raw |>
  mutate(year = as.character(year(date))) |>    # extract year
  left_join(population |> select(state, year, population, region_name),
            by = c("state", "year")) |>         # add population & region info
  filter(year %in% c("2020", "2021"))          # keep 2020–2021 data

cases |>
  filter(year %in% c(2020, 2021)) |>
  ggplot(aes(x = date, y = (cases / population) * 1e5, color = state)) +
  geom_line(alpha = 0.6) +                # plot cases per 100k
  facet_wrap(~ region_name) +             # facet by region
  labs(
    title = "COVID-19 Cases per 100,000 (2020–2021)", # add title & labels
    x = "Date",
    y = "Cases per 100,000"
  ) + 
...
cases |> 
  mutate(
    date  = ymd(date),        # ensure proper date format
    month = month(date, label = TRUE, abbr = FALSE) # extract full month name
  ) |>
  group_by(year, month) |>    # group by year and month
  summarise(total_cases = sum(cases), .groups = "drop") |> 
  arrange(year, month) |>     # order results chronologically
...
deaths <- request(deaths_url) |>
  req_url_query(`$limit` = '10000000000') |>    # request full dataset
  req_perform() |>
  resp_body_json(simplifyDataFrame = TRUE) |>   # parse JSON into data frame
  mutate(
    date   = as.Date(end_date),                 # convert to Date
    deaths = as.numeric(covid_19_deaths)        # ensure numeric deaths
  ) |>
  filter(
    state %in% c(state.name,                    # keep valid states/DC/PR
                 "District of Columbia", "Puerto Rico"),  
    !is.na(date), 
    !is.na(deaths)                              # drop missing values
  )
...
deaths |>
  group_by(state) |>
  summarise(td = sum(deaths, na.rm = TRUE),
            .groups = "drop") |>  # total deaths by state
  mutate(rank = rank(-td)) |>     # rank states by deaths
  filter(rank <= 10) |>           # keep top 10 states
  ggplot(aes(x = reorder(state, td), y = td)) +
  geom_col(stat = "identity", color = "darkgoldenrod", linewidth = 1) + 
  ...
  geom_col(fill = "firebrick")  # fill bars with red
